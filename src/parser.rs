...
impl Parser {
        ...
        match self.peek() {
            ...
            _ => {
                ...
                match self.peek() {
                    ...
                    _ => panic!("Expected '.' or ':-' after atom"),
                }
            }
        }
    }
    ...
    fn parse_atom(&mut self) -> Atom {
        if let Some(Token::Identifier(name)) = self.peek() {
            ...
            panic!("Expected identifier for atom");
        }
    }
        ...
        match self.peek() {
            Some(Token::Identifier(_)) => {
                ...
                if atom.args.is_empty() {
                    Term::Constant(atom.name)
                } else {
                    Term::Compound { name: atom.name, args: atom.args }
                }
            }
...
fn parse_tokens(tokens: Vec<Token>) -> Statement {
...
mod tests {
    ...
    fn test_parse_rule() {
        ...
        let stmt = parse_tokens(tokens);
        assert!(matches!(stmt, Statement::Rule { .. }));
    }
